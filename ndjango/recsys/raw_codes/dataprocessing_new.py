...
# 원본 데이터에 동일한 처리를 한 리스트 생성 (또는 csv 호출)

#enrecipe_cleaned=clean_recipe_dataframe(enrecipe)

enrecipe_cleaned=pd.read_csv('F:\\Ndjango\\branch_test\\enrecipe_cleaned.csv')

threshold=0.5
ing_list=['onion','beef','apple','cabbage'] #input (입력한 list)
# 카테고리 호출
with open('spectralingre23.pickle', 'rb') as handle:
    ing_categories = pickle.load(handle)

#해당 list에 있는 재료로 검색해 index 찾기
...
tv=enrecipe_cleaned.loc[matchesidx,['Title','Instructions','Image_Name','Cleaned_Ingredients']]
print(tv)
...
with open('spectralingre21.pickle', 'rb') as handle:
    ing_categories2 = pickle.load(handle)
t1_keys=pd.DataFrame(list(ing_categories1.keys()))
